...
(() => {
  ...
  const chs = [
    ...
    ["부록", "단계별 산출물", "Appendix · OCPP · ERD · 시퀀스 · 시나리오 · 스토리보드 · 프로토타입", C.muted, "A"],
  ];
...
(() => {
  ...
  const quad = [
    ...
    ["E", "Economic", "경제", C.tealDk, ["휘발유가 상승 → EV 경제성 개선", "충전 단가 ~340원/kWh (저렴)", "드라이버 운영비 40~60% 절감", "GDP 4~5% 성장 · 중산층 확대"]],
    ["S", "Social", "사회", C.amber, ["스마트폰 보급률 ~65%", "EV 등록 연 40%+ 성장", "외국인 여행자 Negative 65.8%", "언어·결제 장벽 = 개선 기회"]],
    ["T", "Technology", "기술", C.amberDk, ["OCPP 1.6J 통신 표준 보편화", "LOCA 충전소 40 → 100개소", "KTR 충전기 안전인증 협력(’26)", "슈퍼앱 통합 기술 성숙"]],
  ];
...
(() => {
  ...
  colWd.forEach((w, i) => { if (i < colWd.length - 1) colX.push(colX[i] + w); });
  const rows = [
    ["LOCA Taxi", "3.43⭐", "37.1%", "40.9%", "447건", "72건", "EV가 이미 핵심 서비스 (언급 43.6%)", "bench"],
...
(() => {
  ...
  s.addText([
    ...
    { text: "앱오류 58.6%는 ", options: { color: C.tealLt } },
...
(() => {
  ...
  const te = [
    ...
    ["뉴스·하드웨어 추가 설계", "‘앱오류 35%’ 원인 규명 위해 HW 뉴스 1,327건 추가 수집"],
  ];
  let ey = ty + 0.78;
  te.forEach(([h, d]) => {
...
// 9. 불만 카테고리
chartSlide("Insight · 02", "분석 ② 불만 카테고리 — 앱오류가 압도적 1위",
  "01_complaint_categories.png", [1486, 736],
  "가장 먼저 고칠 것은 명확하다.\n‘앱오류’ 하나가 전체 불만의 35%를 차지한다.",
  [
...
// 11. 경쟁사 불만 비교
chartSlide("Insight · 04", "분석 ④ 앱별 불만 구조 — 모두 ‘앱오류’가 핵심",
  "11_app_competitive_analysis.png", [1785, 885],
  "경쟁사를 이기는 길은 단순하다.\n모두의 공통 약점 ‘앱 안정성’ 한 축만 잡으면 된다.",
  [
...
// 12. 앱↔HW 연계 (핵심 인사이트)
chartSlide("Key Insight", "핵심 인사이트 — 앱오류 35%, 사실은 통신 문제다",
  "17_app_hw_linkage.png", [2084, 771],
  "‘앱오류 35%’는 앱 버그가 아니라\n앱↔충전기 OCPP 통신 실패다. 그래서 HW까지 함께 설계했다.",
  [
...
(() => {
  ...
  const ps = [
    {
      ...
      demo: ["라이드헤일링 생계 운전자", "하루 2~3회 충전 · 낮 시간대", "LOCA Taxi EV 언급 43.6%"],
      goals: ["빠른 충전 시작", "안정적 결제", "충전소 위치 즉시 확인"],
      pains: ["충전 실패(앱오류)로 영업 손실", "결제 오류 반복", "배차·충전 대기 시간"],
      src: "근거 · LOCA Taxi 447건 + 유튜브 자막",
    },
    {
      name: "외국인 여행자", tag: "성장 타겟", col: C.teal,
      demo: ["단기 체류 · 렌트 EV/스쿠터", "영어 사용 · 해외 카드 보유", "영어 리뷰 Negative 65.8%"],
  ...
  const gap = 0.3, cw = (W - 2 * M - 2 * gap) / 3, x0 = M, cy = 1.82, ch = 4.65;
  ...
  ps.forEach((p, i) => {
    const x = x0 + i * (cw + gap);
    card(s, x, cy, cw, ch);
    s.addShape(pres.shapes.RECTANGLE, { x, y: cy, w: cw, h: 0.78, fill: { color: p.col } });
    s.addText(p.name, { x: x + 0.24, y: cy + 0.08, w: cw - 0.45, h: 0.4, fontFace: HF, fontSize: 14.5, bold: true, color: C.white, valign: "middle", margin: 0 });
    s.addShape(pres.shapes.ROUNDED_RECTANGLE, { x: x + 0.24, y: cy + 0.46, w: 1.5, h: 0.26, rectRadius: 0.05, fill: { color: "FFFFFF" } });
    s.addText(p.tag, { x: x + 0.24, y: cy + 0.46, w: 1.5, h: 0.26, fontFace: BF, fontSize: 9.5, bold: true, color: p.col, align: "center", valign: "middle", margin: 0 });
    block(s, x + 0.24, cy + 0.95, cw - 0.46, "인구통계 · 특성", p.col, p.demo);
    block(s, x + 0.24, cy + 2.12, cw - 0.46, "목표 (Goals)", C.tealDk, p.goals);
    block(s, x + 0.24, cy + 3.28, cw - 0.46, "좌절 요인 (Frustrations)", C.red, p.pains);
    s.addText(p.src, { x: x + 0.24, y: cy + ch - 0.32, w: cw - 0.46, h: 0.26, fontFace: BF, fontSize: 8.5, italic: true, color: C.muted, margin: 0 });
  });
})();
...
(() => {
  ...
  const map = [
    ["앱오류 35%", "5,417건", "충전 세션 자동 복구", "P0", C.red],
    ["결제오류 8.2%", "1,266건", "RFID 오프라인 결제 · 결제 확인 화면", "P1", C.amber],
    ["연결실패 + 실사용", "유튜브 사례", "QR 재시도 + 수동 ID 입력", "P0", C.red],
    ["OCPP 호환 169건", "HW 뉴스", "OCPP 2.0.1 준수 검증 · HW 모니터링", "P0", C.red],
    ["충전소위치 1.8%", "285건", "실시간 충전소 지도", "P2", C.teal],
  ];
  // header
...
(() => {
  ...
  const stages = [
    ...
    ["QR 스캔", "충전기 QR", 0.2, 0.82, "QR 인식 실패·앱 멈춤 (앱오류 35%)", "[F2] 자동 재시도 + 수동 입력"],
    ["결제", "결제 화면", 0.25, 0.82, "결제 후 충전 안 됨 (결제오류 8.2%)", "[F4] RFID · [F5] 결제 확인"],
...
divider("4", "05", "기대효과 & 회고", "Impact & Retrospective", "측정 가능한 목표와 배운 것");
...
(() => {
  ...
  const ks = [
    ...
    ["35%", "→ ½", "앱오류 리뷰", "세션 자동복구 효과", C.red],
    ["169건", "→ 표준화", "OCPP 호환 이슈", "2.0.1 준수 검증", C.tealDk],
  ];
...
(() => {
  ...
  s.addText("데이터 분석 → 기획 → 설계 전 과정에서 실제로 만든 산출물입니다.", { x: M + 0.1, y: 3.0, w: 11, h: 0.4, fontFace: BF, fontSize: 14, color: C.tealLt, margin: 0 });
