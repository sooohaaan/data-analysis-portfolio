...
(() => {
  ...
  s.addShape(pres.shapes.RECTANGLE, { x: tx, y: ty, w: tw, h: 0.46, fill: { color: C.dark } });
  heads.forEach((h, i) => s.addText(h, { x: cx[i] + 0.12, y: ty, w: cwd[i] - 0.2, h: 0.46, fontFace: HF, fontSize: 11, bold: true, color: i === 0 ? C.tealLt : C.white, valign: "middle", margin: 0 }));
  let ry = ty + 0.46;
  const rh = 0.43;
  rows.forEach((r, i) => {
...
(() => {
  ...
  // bottom: 교훈
  card(s, M, cy + ch + 0.2, W - 2 * M, 1.55, C.dark, false);
  s.addShape(pres.shapes.RECTANGLE, { x: M, y: cy + ch + 0.2, w: 0.16, h: 1.55, fill: { color: C.amber } });
  s.addText([
    ...
    { text: "단, Xanh SM(Vingroup)이 바로 그 틈을 EV 라이드헤일링으로 노린다", options: { bold: true, color: C.tealLt } },
...
(() => {
  ...
  let pry = ty + 0.62;
  heads.forEach((h, i) => s.addText(h, { x: ccx[i], y: pry, w: cwd[i], h: 0.32, fontFace: BF, fontSize: 10.5, bold: true, color: C.tealLt, align: i ? "center" : "left", valign: "middle", margin: 0 }));
  ...
  // bottom insight band
  card(s, M, ty + 2.75, W - 2 * M, 1.55, C.dark, false);
  s.addShape(pres.shapes.RECTANGLE, { x: M, y: ty + 2.75, w: 0.16, h: 1.55, fill: { color: C.amber } });
  s.addText([
    { text: "결론 — 마진은 구조적으로 높다(84%). 변수는 ‘가동률’이다.\n", options: { bold: true, color: C.amber } },
    { text: "그리고 ", options: { color: C.white } },
    { text: "KOKKOK Move 드라이버(현지 라이드헤일링 운영)", options: { bold: true, color: C.tealLt } },
...
(() => {
  ...
  card(s, M, cy + ch + 0.18, W - 2 * M, 1.15, C.dark, false);
...
(() => {
  ...
  // Ask band
  card(s, M, cy + ch + 0.18, W - 2 * M, 1.15, C.dark, false);
  s.addShape(pres.shapes.RECTANGLE, { x: M, y: cy + ch + 0.18, w: 0.16, h: 1.15, fill: { color: C.amber } });
  s.addText([
    { text: "제안(Ask) — Phase 0 파일럿 승인:  ", options: { bold: true, color: C.amber } },
    ...
    { text: "※ 규모는 가정·제안치.", options: { color: C.tealLt, italic: true } },
...
(() => {
  ...
  s.addShape(pres.shapes.RECTANGLE, { x: tx, y: ty, w: tw, h: 0.42, fill: { color: C.dark } });
  heads.forEach((h, i) => s.addText(h, { x: cx[i] + 0.1, y: ty, w: cwd[i] - 0.16, h: 0.42, fontFace: HF, fontSize: 9.5, bold: true, color: i === 0 ? C.tealLt : C.white, valign: "middle", margin: 0 }));
  let ry = ty + 0.42;
  const rh = 0.62;
  rows.forEach((r, i) => {
    s.addShape(pres.shapes.RECTANGLE, { x: tx, y: ry, w: tw, h: rh, fill: { color: i % 2 ? "EAF1EF" : C.white }, line: { color: C.line, width: 0.6 } });
