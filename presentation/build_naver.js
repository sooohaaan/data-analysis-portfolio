...
const C = {
  // NAVER 테마: 흰 배경 + 단일 그린 액센트 + 검정/회색
  dark:   "1A1A1A", // 다크/콜아웃 패널 → 근검정
  dark2:  "C9F2D8", // 글로우 틴트 → 라이트 그린
  teal:   "03C75A", // NAVER 그린 (단일 액센트)
  tealDk: "02A14B",
  tealLt: "C9F2D8",
  amber:  "03C75A", // 앰버 → 그린으로 통일(모노그린)
  amberDk:"02A14B",
  red:    "EB5757", // 부정 의미만 (소프트 레드)
  ink:    "1A1A1A", // 검정 텍스트
  muted:  "8C8C8C", // 회색
  light:  "FFFFFF", // 콘텐츠 배경 흰색
  card:   "FFFFFF",
  line:   "E6E6E6", // 헤어라인
  white:  "FFFFFF",
};
...
pres.title = "KOKKOK EV — 데이터로 문제를 풀어 서비스를 만드는 기획자";
...
(() => {
  ...
  const quad = [
    ["P", "Political", "정치·정책", C.teal, ["내연기관차 수입 잠정 중단 (2026.6~12월)", "EV 소비세 면제·0% 수입관세·등록세 30%↓", "정부 목표 2030년 EV 30%·충전소 500개", "MOIC·MEM 주도(.gov.la 공표) → 강한 친EV"]],
    ["E", "Economic", "경제", C.tealDk, ["휘발유 23,834₭/L vs 충전 4,000₭/kWh", "전기 ~647₭/kWh·전력 잉여 → 충전 마진 우위", "가구 월소비 중위 122.6만₭ → 가격 민감", "2026 연료위기(디젤 급등) = 전환 가속"]],
    ["S", "Social", "사회", C.amber, ["스마트폰 ~85% · 인터넷 66%", "빈곤율 15%↓ · 15~39세 43.2% 젊은 층", "외국인 여행자 Negative 65.8%", "언어·결제 장벽 = 개선 기회"]],
...
(() => {
  ...
  card(s, rx, fy, rw, 4.55, C.white, false);
  s.addShape(pres.shapes.RECTANGLE, { x: rx, y: fy, w: 0.14, h: 4.55, fill: { color: C.teal } });
  s.addText("왜 지금인가 — 진입 논거", { x: rx + 0.4, y: fy + 0.22, w: rw - 0.7, h: 0.4, fontFace: HF, fontSize: 16, bold: true, color: C.ink, margin: 0 });
  const concl = [
  ...
  let cy2 = fy + 0.72;
  concl.forEach((t) => {
    s.addShape(pres.shapes.OVAL, { x: rx + 0.4, y: cy2 + 0.05, w: 0.16, h: 0.16, fill: { color: C.teal } });
    s.addText(t, { x: rx + 0.7, y: cy2 - 0.05, w: rw - 1.0, h: 0.95, fontFace: BF, fontSize: 12, color: C.ink, lineSpacingMultiple: 1.12, valign: "top", margin: 0 });
    cy2 += 0.9;
  });
  s.addText("※ 데이터 신뢰도 원칙: 시장조사업체 추정치는 검증 실패로 전량 제외. IEA·EY-Parthenon·기업 공식 발표만 인용.", {
    x: rx + 0.4, y: fy + 4.0, w: rw - 0.7, h: 0.5, fontFace: BF, fontSize: 9.5, italic: true, color: C.muted, lineSpacingMultiple: 1.05, valign: "top", margin: 0,
  });
})();

// ===================================================================
// 3.65 [거시] 분석 대상 기업 — 우리가 데이터를 수집한 곳
// ===================================================================
(() => {
  const s = pres.addSlide();
  chrome(s, "Market · Competitors", "분석 대상 기업 — 데이터를 수집한 9개 서비스");
  const tx = M, ty = 1.78, tw = W - 2 * M;
  const heads = ["기업 / 서비스", "국가", "유형", "수집 데이터", "본 분석에서의 역할"];
  const cwd = [2.55, 1.7, 2.25, 1.75, tw - 2.55 - 1.7 - 2.25 - 1.75];
  const cx = [tx]; cwd.forEach((w, i) => { if (i < cwd.length - 1) cx.push(cx[i] + w); });
  const rows = [
    ["LOCA EV", "라오스", "EV 충전", "앱리뷰 7", "현지 충전 벤치마크 (경영진 지정)", C.teal],
    ["LOCA Taxi", "라오스", "라이드헤일링", "슈퍼앱 447", "라오스 모빌리티 1위 (사업모델)", C.teal],
    ["KOKKOK Move", "라오스", "라이드헤일링·자사", "슈퍼앱 378", "우리 서비스 기반 (확장 출발점)", C.teal],
    ["PTT blueplus+", "태국", "EV 충전", "앱 1,518", "충전 UX 벤치마크 (최고 3.11점)", C.amber],
    ["PEA VOLTA", "태국", "EV 충전", "앱 499", "충전 비교군", C.amber],
    ["EleXA", "태국", "EV 충전", "앱 263", "충전 비교군", C.amber],
    ["Green SM (Xanh SM)", "베트남", "EV 라이드헤일링", "앱 26,603", "VOC 본체 · 역내 위협(라오스 진입)", C.red],
    ["Grab", "역내 8국 (라오스 X)", "라이드헤일링 슈퍼앱", "슈퍼앱 1,479", "역내 슈퍼앱 벤치마크", C.muted],
    ["Gojek", "인니 등 역내", "라이드헤일링 슈퍼앱", "슈퍼앱 1,066", "슈퍼앱 비교군", C.muted],
  ];
  s.addShape(pres.shapes.RECTANGLE, { x: tx, y: ty, w: tw, h: 0.46, fill: { color: C.teal } });
  heads.forEach((h, i) => s.addText(h, { x: cx[i] + 0.12, y: ty, w: cwd[i] - 0.2, h: 0.46, fontFace: HF, fontSize: 11, bold: true, color: C.white, valign: "middle", margin: 0 }));
  let ry = ty + 0.46;
  const rh = 0.43;
  rows.forEach((r, i) => {
    s.addShape(pres.shapes.RECTANGLE, { x: tx, y: ry, w: tw, h: rh, fill: { color: i % 2 ? "EAF1EF" : C.white }, line: { color: C.line, width: 0.6 } });
    s.addShape(pres.shapes.RECTANGLE, { x: tx, y: ry, w: 0.07, h: rh, fill: { color: r[5] } });
    s.addText(r[0], { x: cx[0] + 0.16, y: ry, w: cwd[0] - 0.24, h: rh, fontFace: BF, fontSize: 10.5, bold: true, color: C.ink, valign: "middle", margin: 0 });
    s.addText(r[1], { x: cx[1] + 0.1, y: ry, w: cwd[1] - 0.18, h: rh, fontFace: BF, fontSize: 10, color: C.muted, valign: "middle", margin: 0 });
    s.addText(r[2], { x: cx[2] + 0.1, y: ry, w: cwd[2] - 0.18, h: rh, fontFace: BF, fontSize: 10, color: C.ink, valign: "middle", margin: 0 });
    s.addText(r[3], { x: cx[3] + 0.1, y: ry, w: cwd[3] - 0.18, h: rh, fontFace: HF, fontSize: 10.5, bold: true, color: r[5] === C.muted ? C.tealDk : r[5], valign: "middle", margin: 0 });
    s.addText(r[4], { x: cx[4] + 0.1, y: ry, w: cwd[4] - 0.18, h: rh, fontFace: BF, fontSize: 10, color: C.ink, valign: "middle", margin: 0 });
    ry += rh;
  });
  s.addText([
    { text: "수집 설계 — ", options: { bold: true, color: C.teal } },
    { text: "라오스 현지 3(타깃 시장) · 태국 충전 3(UX 벤치마크) · 베트남/역내 슈퍼앱 3(EV 모빌리티·위협). ", options: { color: C.muted } },
    { text: "충전 UX는 태국에서, 사업모델·현지 경쟁은 라오스에서, 슈퍼앱 동학은 역내에서 학습.", options: { color: C.ink } },
  ], { x: tx, y: ry + 0.16, w: tw, h: 0.6, fontFace: BF, fontSize: 11, valign: "top", lineSpacingMultiple: 1.12, margin: 0 });
})();
...
(() => {
  ...
  // header
  s.addShape(pres.shapes.RECTANGLE, { x: tx, y: ty, w: tw, h: 0.5, fill: { color: C.teal } });
  heads.forEach((h, i) => s.addText(h, { x: colX[i] + 0.1, y: ty, w: colWd[i] - 0.18, h: 0.5, fontFace: HF, fontSize: 11.5, bold: true, color: C.white, valign: "middle", align: i === 0 || i === 5 ? "left" : "center", margin: 0 }));
  let ry = ty + 0.5;
  const rh = 0.74;
  rows.forEach((r, i) => {
  ...
  // bottom insight band
  card(s, M, ry + 0.12, tw, 1.0, C.teal, false);
  s.addShape(pres.shapes.RECTANGLE, { x: M, y: ry + 0.12, w: 0.14, h: 1.0, fill: { color: C.white } });
  s.addText([
    { text: "시사점  ", options: { bold: true, color: C.white } },
    { text: "전체 평균 2점대 레드오션 — PTT(3.11⭐)가 유일한 벤치마크. 라오스 직접 경쟁은 LOCA EV·Green SM뿐.\n", options: { color: C.white } },
    { text: "⚠ 데이터 보강  ", options: { bold: true, color: "EAFBF1" } },
    { text: "점유율 정량(%)은 검증 자료 부재 → 리뷰수를 인지도 대용으로 사용. 공식 단가 수집: PTT 4.5~7.5฿·PEA 5.3~8.8฿·EleX 7.5฿·V-Green 3,858₫·LOCA 4,300₭/kWh.", options: { color: "EAFBF1" } },
...
(() => {
  ...
  s.addShape(pres.shapes.RECTANGLE, { x: tx, y: ty, w: tw, h: 0.5, fill: { color: C.teal } });
  heads.forEach((h, i) => s.addText(h, { x: colX[i] + 0.1, y: ty, w: colWd[i] - 0.18, h: 0.5, fontFace: HF, fontSize: 11.5, bold: true, color: C.white, valign: "middle", align: i === 0 || i === 6 ? "left" : "center", margin: 0 }));
  let ry = ty + 0.5;
  const rh = 0.72;
  rows.forEach((r, i) => {
  ...
  // bottom insight band
  card(s, M, ry + 0.14, tw, 1.55, C.teal, false);
  s.addShape(pres.shapes.RECTANGLE, { x: M, y: ry + 0.14, w: 0.14, h: 1.55, fill: { color: C.white } });
  const pts = [
  ...
  let py = ry + 0.3;
  pts.forEach(([h, d]) => {
    s.addShape(pres.shapes.OVAL, { x: M + 0.42, y: py + 0.04, w: 0.14, h: 0.14, fill: { color: C.white } });
    s.addText([{ text: h + " — ", options: { bold: true, color: "EAFBF1" } }, { text: d, options: { color: C.white } }],
...
chartSlide("Market · Competitors", "경쟁사 분석 ③ — 라오스 현지 뉴스 점유율로 찾은 진짜 경쟁자",
  "19_laos_share_of_voice_nv.png", [1156, 599],
...
// ===================================================================
// 3.95 [거시] 시장 진입 교훈 — Grab은 왜 라오스를 비웠나
// ===================================================================
(() => {
  const s = pres.addSlide();
  chrome(s, "Market Lesson", "시장 진입 교훈 — Grab은 왜 라오스를 비웠나");
  const cy = 1.85, ch = 2.7, gap = 0.3, cw = (W - 2 * M - gap) / 2;
  // left: Grab 역내 공세
  card(s, M, cy, cw, ch);
  s.addText("Grab의 동남아 EV 공세", { x: M + 0.3, y: cy + 0.18, w: cw - 0.6, h: 0.4, fontFace: HF, fontSize: 15, bold: true, color: C.ink, margin: 0 });
  s.addText([
    { text: "• 동남아 8개국 운영 (싱가포르·인니·필리핀·말련·태국·베트남·미얀마·캄보디아)\n", options: {} },
    { text: "• BYD 5만 대 + GAC 2만 대 = ", options: {} },
    { text: "7만 대 EV 플릿", options: { bold: true, color: C.tealDk } },
    { text: " 파트너십(2025)\n", options: {} },
    { text: "• 대상 6개국 — ", options: {} },
    { text: "라오스는 모두 제외", options: { bold: true, color: C.red } },
  ], { x: M + 0.3, y: cy + 0.66, w: cw - 0.6, h: ch - 0.9, fontFace: BF, fontSize: 11.5, color: C.ink, lineSpacingMultiple: 1.3, valign: "top", margin: 0 });
  // right: 왜 라오스만
  const rx = M + cw + gap;
  card(s, rx, cy, cw, ch, C.card);
  s.addText("왜 라오스만 비었나 (추정)", { x: rx + 0.3, y: cy + 0.18, w: cw - 0.6, h: 0.4, fontFace: HF, fontSize: 15, bold: true, color: C.ink, margin: 0 });
  s.addText([
    { text: "• 소규모 시장 (인구 ~765만) → 진입 대비 수익성\n", options: {} },
    { text: "• LOCA의 현지 선점 (택시+결제+충전 수직통합)\n", options: {} },
    { text: "• 디지털결제·스마트폰 보급 economics\n", options: {} },
    { text: "※ Grab 공식 사유는 미발표 — 추정", options: { italic: true, color: C.muted } },
  ], { x: rx + 0.3, y: cy + 0.66, w: cw - 0.6, h: ch - 0.9, fontFace: BF, fontSize: 11.5, color: C.ink, lineSpacingMultiple: 1.3, valign: "top", margin: 0 });
  // bottom: 교훈
  card(s, M, cy + ch + 0.2, W - 2 * M, 1.55, C.teal, false);
  s.addShape(pres.shapes.RECTANGLE, { x: M, y: cy + ch + 0.2, w: 0.16, h: 1.55, fill: { color: C.white } });
  s.addText([
    { text: "교훈 — 역내 1위 Grab도 라오스는 진입하지 않았다 = 글로벌 자본이 비워둔 틈.\n", options: { bold: true, color: C.white } },
    { text: "현지 슈퍼앱(LOCA·KOKKOK)이 선점할 니치다. ", options: { color: C.white } },
    { text: "단, Xanh SM(Vingroup)이 바로 그 틈을 EV 라이드헤일링으로 노린다", options: { bold: true, color: "EAFBF1" } },
    { text: " → ‘작아서 안전’이 아니라 ‘먼저 잡아야 하는’ 시장. 골든타임 12~18개월의 또 다른 근거.", options: { color: C.white } },
  ], { x: M + 0.45, y: cy + ch + 0.36, w: W - 2 * M - 0.9, h: 1.3, fontFace: BF, fontSize: 12.5, lineSpacingMultiple: 1.2, valign: "top", margin: 0 });
})();

divider("1", "02", "문제 정의", "Problem Definition", "데이터로 검증한다 — 무엇을 풀 것인가");
...
(() => {
  ...
  // insight band
  card(s, M, 4.05, W - 2 * M, 2.3, C.teal, false);
  s.addShape(pres.shapes.RECTANGLE, { x: M, y: 4.05, w: 0.16, h: 2.3, fill: { color: C.white } });
  s.addText("핵심 가설", { x: M + 0.45, y: 4.28, w: 4, h: 0.35, fontFace: BF, fontSize: 12, bold: true, color: "EAFBF1", charSpacing: 2, margin: 0 });
  s.addText([
    { text: "“EV는 빠르게 들어오는데 충전 경험은 별점 2.56점.\n", options: { color: C.white, bold: true } },
    { text: "충전앱 불만(앱 불안정 27.6%)은 ", options: { color: "EAFBF1" } },
    { text: "위협이 아니라 기회", options: { color: C.white, bold: true } },
    { text: " — 3점대 서비스만 만들어도 동남아 2위권 진입이 가능하다.”", options: { color: "EAFBF1" } },
...
(() => {
  ...
  card(s, rx, ty, rw, 4.6, C.white);
  s.addShape(pres.shapes.RECTANGLE, { x: rx, y: ty, w: 0.14, h: 4.6, fill: { color: C.teal } });
  s.addText("수집 단계에서 부딪힌 현실", { x: rx + 0.4, y: ty + 0.2, w: rw - 0.7, h: 0.4, fontFace: HF, fontSize: 15, bold: true, color: C.ink, margin: 0 });
  const te = [
  ...
  let ey = ty + 0.78;
  te.forEach(([h, d]) => {
    s.addText("✦", { x: rx + 0.4, y: ey, w: 0.35, h: 0.3, fontFace: BF, fontSize: 13, bold: true, color: C.teal, margin: 0 });
    s.addText([
      { text: h + "\n", options: { bold: true, color: C.tealDk, fontSize: 12.5 } },
      { text: d, options: { color: C.muted, fontSize: 11 } },
...
(() => {
  ...
  // 6x badge
  s.addShape(pres.shapes.ROUNDED_RECTANGLE, { x: bx + bw - 1.78, y: by + 1.45, w: 1.4, h: 1.32, rectRadius: 0.1, fill: { color: C.teal } });
  s.addText([{ text: "6×\n", options: { fontSize: 26, bold: true, color: C.white } }, { text: "처리 시간 단축", options: { fontSize: 9.5, color: "EAFBF1" } }],
  ...
  card(s, rx, by, rw, 4.6, C.white);
  s.addShape(pres.shapes.RECTANGLE, { x: rx, y: by, w: 0.14, h: 4.6, fill: { color: C.teal } });
  s.addText("엔지니어링 트러블슈팅", { x: rx + 0.4, y: by + 0.2, w: rw - 0.7, h: 0.4, fontFace: HF, fontSize: 15, bold: true, color: C.ink, margin: 0 });
  let iy = by + 0.82;
  items.forEach(([h, d]) => {
    // NAVER grid row: hairline separator + green index tick + ink title + grey detail
    if (iy > by + 0.82) s.addShape(pres.shapes.LINE, { x: rx + 0.4, y: iy - 0.12, w: rw - 0.8, h: 0, line: { color: C.line, width: 0.75 } });
    s.addShape(pres.shapes.RECTANGLE, { x: rx + 0.4, y: iy + 0.02, w: 0.1, h: 0.62, fill: { color: C.teal } });
    s.addText([
      { text: h + "\n", options: { bold: true, color: C.ink, fontSize: 12.5 } },
      { text: d, options: { color: C.muted, fontSize: 11 } },
    ], { x: rx + 0.66, y: iy - 0.02, w: rw - 0.98, h: 0.92, fontFace: BF, lineSpacingMultiple: 1.08, valign: "top", margin: 0 });
    iy += 0.95;
  });
})();
...
function chartSlide(kicker, title, img, imgWH, punch, takeaways) {
  ...
  card(s, rx, 1.78, rw, 4.7, C.white);
  s.addShape(pres.shapes.RECTANGLE, { x: rx, y: 1.78, w: 0.14, h: 4.7, fill: { color: C.teal } });
  // 핵심 메시지 (so-what)
  s.addText("핵심 메시지", { x: rx + 0.42, y: 1.96, w: rw - 0.7, h: 0.3, fontFace: BF, fontSize: 11.5, bold: true, color: C.teal, charSpacing: 2, margin: 0 });
  s.addShape(pres.shapes.ROUNDED_RECTANGLE, { x: rx + 0.42, y: 2.28, w: rw - 0.84, h: 1.66, rectRadius: 0.06, fill: { color: C.tealLt } });
  s.addText(punch, { x: rx + 0.62, y: 2.34, w: rw - 1.24, h: 1.54, fontFace: HF, fontSize: 14, bold: true, color: C.tealDk, lineSpacingMultiple: 1.16, valign: "middle", margin: 0 });
  // 근거
  s.addText("근거", { x: rx + 0.42, y: 4.04, w: rw - 0.7, h: 0.3, fontFace: BF, fontSize: 11.5, bold: true, color: C.muted, charSpacing: 2, margin: 0 });
  let ty = 4.4;
  takeaways.forEach((t) => {
    const lines = Math.min(2, Math.max(1, Math.ceil(t.length / 22)));
    s.addShape(pres.shapes.OVAL, { x: rx + 0.42, y: ty + 0.04, w: 0.13, h: 0.13, fill: { color: C.teal } });
    s.addText(t, { x: rx + 0.68, y: ty - 0.05, w: rw - 1.0, h: lines * 0.28 + 0.1, fontFace: BF, fontSize: 11, color: C.ink, lineSpacingMultiple: 1.06, valign: "top", margin: 0 });
    ty += 0.25 * lines + 0.11;
  });
  return s;
}
...
function artifactSlide(d) {
  ...
  s.addText("무엇인가", { x: rx + 0.42, y: iy + 1.05, w: rw - 0.7, h: 0.32, fontFace: BF, fontSize: 11.5, bold: true, color: C.teal, charSpacing: 1, margin: 0 });
  s.addText(d.desc, { x: rx + 0.42, y: iy + 1.4, w: rw - 0.75, h: 1.35, fontFace: BF, fontSize: 13, color: C.ink, lineSpacingMultiple: 1.2, valign: "top", margin: 0 });
  s.addShape(pres.shapes.LINE, { x: rx + 0.42, y: iy + 2.82, w: rw - 0.8, h: 0, line: { color: C.line, width: 1 } });
  s.addText("역할 · 근거", { x: rx + 0.42, y: iy + 2.96, w: rw - 0.7, h: 0.32, fontFace: BF, fontSize: 11.5, bold: true, color: C.teal, charSpacing: 1, margin: 0 });
  s.addText(d.role, { x: rx + 0.42, y: iy + 3.31, w: rw - 0.75, h: 1.05, fontFace: BF, fontSize: 12.5, color: C.ink, lineSpacingMultiple: 1.2, valign: "top", margin: 0 });
  if (d.note) s.addText(d.note, { x: rx + 0.42, y: iy + ih - 0.62, w: rw - 0.75, h: 0.5, fontFace: BF, fontSize: 10, italic: true, color: C.muted, lineSpacingMultiple: 1.05, valign: "top", margin: 0 });
}
...
chartSlide("Insight · 02", "분석 ② 불만 구조 — 도메인을 나누니 진짜 문제가 보였다",
  "18_complaint_2layer_nv.png", [1936, 787],
...
chartSlide("Insight · 03", "분석 ③ 경쟁 포지셔닝 — 비어 있는 ‘오른쪽 아래’",
  "10_app_positioning_map_nv.png", [1155, 700],
...
chartSlide("Insight · 04", "분석 ④ 충전앱 레이어 — 우리가 풀 문제",
  "11_app_competitive_analysis_nv.png", [1415, 692],
...
chartSlide("Key Insight", "핵심 인사이트 — 가설을 데이터로 뒤집다",
  "17_app_hw_linkage_nv.png", [1676, 733],
...
chartSlide("Insight · HW", "하드웨어 이슈 맵 — 빈도 높고 심각한 문제 식별",
  "14_hw_positioning_map_nv.png", [1148, 700],
...
(() => {
  ...
  ps.forEach((p, i) => {
    const x = x0 + i * (cw + gap);
    card(s, x, cy, cw, ch);
    // NAVER grid header: green left accent + ink name + tag chip (no heavy color band)
    s.addShape(pres.shapes.RECTANGLE, { x, y: cy, w: 0.12, h: 0.94, fill: { color: p.col } });
    s.addText(p.name, { x: x + 0.28, y: cy + 0.13, w: cw - 0.5, h: 0.34, fontFace: HF, fontSize: 13.5, bold: true, color: C.ink, valign: "middle", margin: 0 });
    s.addShape(pres.shapes.ROUNDED_RECTANGLE, { x: x + 0.28, y: cy + 0.5, w: 1.3, h: 0.26, rectRadius: 0.05, fill: { color: p.col } });
    s.addText(p.tag, { x: x + 0.28, y: cy + 0.5, w: 1.3, h: 0.26, fontFace: BF, fontSize: 9, bold: true, color: C.white, align: "center", valign: "middle", margin: 0 });
    s.addText(p.who, { x: x + 1.66, y: cy + 0.5, w: cw - 1.9, h: 0.26, fontFace: BF, fontSize: 9.5, color: C.muted, valign: "middle", margin: 0 });
    s.addShape(pres.shapes.LINE, { x: x + 0.24, y: cy + 0.96, w: cw - 0.46, h: 0, line: { color: C.line, width: 0.75 } });
    s.addText(p.quote, { x: x + 0.24, y: cy + 1.04, w: cw - 0.46, h: 0.4, fontFace: BF, fontSize: 10, italic: true, color: p.col, valign: "middle", lineSpacingMultiple: 1.0, margin: 0 });
    block(s, x + 0.24, cy + 1.52, cw - 0.46, "인구통계 · 특성", p.col, p.demo);
    s.addShape(pres.shapes.LINE, { x: x + 0.24, y: cy + 2.5, w: cw - 0.46, h: 0, line: { color: C.line, width: 0.75 } });
    block(s, x + 0.24, cy + 2.6, cw - 0.46, "목표 (Goals)", C.tealDk, p.goals);
    s.addShape(pres.shapes.LINE, { x: x + 0.24, y: cy + 3.58, w: cw - 0.46, h: 0, line: { color: C.line, width: 0.75 } });
    block(s, x + 0.24, cy + 3.68, cw - 0.46, "좌절 요인 (Frustrations)", C.red, p.pains);
    s.addText([{ text: "규모(추정) ", options: { bold: true, color: p.col } }, { text: p.size, options: { color: C.ink } }], { x: x + 0.24, y: cy + ch - 0.62, w: cw - 0.46, h: 0.3, fontFace: BF, fontSize: 8.5, valign: "middle", lineSpacingMultiple: 1.0, margin: 0 });
...
(() => {
  ...
  // bottom takeaway
  card(s, M, 6.18, W - 2 * M, 0.6, C.teal, false);
  s.addShape(pres.shapes.RECTANGLE, { x: M, y: 6.18, w: 0.14, h: 0.6, fill: { color: C.white } });
  s.addText([
...
(() => {
  ...
  card(s, M, 6.32, W - 2 * M, 0.6, C.teal, false);
  s.addShape(pres.shapes.RECTANGLE, { x: M, y: 6.32, w: 0.14, h: 0.6, fill: { color: C.white } });
  s.addText([
    { text: "진행 순서  ", options: { bold: true, color: C.white } },
...
(() => {
  ...
  s.addShape(pres.shapes.RECTANGLE, { x: tx, y: ty, w: tw, h: 0.55, fill: { color: C.teal } });
  heads.forEach((h, i) => s.addText(h, { x: cx[i] + 0.15, y: ty, w: cols[i] - 0.2, h: 0.55, fontFace: BF, fontSize: 12.5, bold: true, color: C.white, valign: "middle", align: i === 1 || i === 3 ? "center" : "left", margin: 0 }));
  let ry = ty + 0.55;
  const rh = 0.82;
...
(() => {
  ...
  card(s, rx, py0, rw, py1 - py0 + 0.3, C.white);
  s.addShape(pres.shapes.RECTANGLE, { x: rx, y: py0, w: 0.14, h: py1 - py0 + 0.3, fill: { color: C.teal } });
  s.addText("KOKKOK 목표 좌표", { x: rx + 0.38, y: py0 + 0.22, w: rw - 0.6, h: 0.4, fontFace: HF, fontSize: 16, bold: true, color: C.ink, margin: 0 });
  const ins = [
  ...
  let iy = py0 + 0.78;
  ins.forEach((t) => {
    s.addShape(pres.shapes.OVAL, { x: rx + 0.38, y: iy + 0.05, w: 0.15, h: 0.15, fill: { color: C.teal } });
    s.addText(t, { x: rx + 0.66, y: iy - 0.05, w: rw - 1.0, h: 0.7, fontFace: BF, fontSize: 12, color: C.ink, lineSpacingMultiple: 1.12, valign: "top", margin: 0 });
    iy += 0.74;
  });
  s.addText("※ 가격축은 공식 단가(태국 3사·V-Green·LOCA 수집) 기준, 품질·기능축은 별점·기능 수 기반 정성 평가.", {
    x: rx + 0.38, y: py1 - 0.35, w: rw - 0.6, h: 0.5, fontFace: BF, fontSize: 9.5, italic: true, color: C.muted, lineSpacingMultiple: 1.05, valign: "top", margin: 0,
  });
})();
...
(() => {
  ...
  s.addShape(pres.shapes.RECTANGLE, { x: tx, y: ty, w: tw, h: 0.42, fill: { color: C.teal } });
  heads.forEach((h, i) => s.addText(h, { x: colX[i] + 0.1, y: ty, w: colWd[i] - 0.16, h: 0.42, fontFace: HF, fontSize: 11, bold: true, color: i === 4 ? C.white : C.white, valign: "middle", align: (i === 0 || i === 4) ? "center" : "left", margin: 0 }));
  let ry = ty + 0.42;
  const rh = 0.355;
  rows.forEach((r, i) => {
...
(() => {
  ...
  card(s, M, 6.2, W - 2 * M, 0.55, C.teal, false);
  s.addShape(pres.shapes.RECTANGLE, { x: M, y: 6.2, w: 0.14, h: 0.55, fill: { color: C.white } });
  s.addText([
    { text: "핵심  ", options: { bold: true, color: C.white } },
...
(() => {
  ...
  card(s, M, 5.85, W - 2 * M, 0.78, C.teal, false);
  s.addText([
    { text: "오류는 예외가 아니라 기본값  ", options: { bold: true, color: C.white } },
...
(() => {
  ...
  s.addShape(pres.shapes.RECTANGLE, { x: tx, y: ty, w: tw, h: 0.55, fill: { color: C.teal } });
  heads.forEach((h, i) => s.addText(h, { x: colX[i] + 0.1, y: ty, w: colWd[i] - 0.15, h: 0.55, fontFace: HF, fontSize: 12.5, bold: true, color: C.white, valign: "middle", align: i === 0 ? "left" : "center", margin: 0 }));
  let ry = ty + 0.55;
  const rh = 0.74;
  rows.forEach((r, i) => {
    s.addShape(pres.shapes.RECTANGLE, { x: tx, y: ry, w: tw, h: rh, fill: { color: i % 2 ? "EAF1EF" : C.white }, line: { color: C.line, width: 0.75 } });
  ...
  // right insight
  const rx = tx + tw + 0.3, rw = W - M - rx;
  card(s, rx, ty, rw, 0.55 + 5 * rh, C.white);
  s.addShape(pres.shapes.RECTANGLE, { x: rx, y: ty, w: 0.14, h: 0.55 + 5 * rh, fill: { color: C.teal } });
  s.addText("왜 직접 검증했나", { x: rx + 0.38, y: ty + 0.22, w: rw - 0.6, h: 0.4, fontFace: HF, fontSize: 14.5, bold: true, color: C.ink, margin: 0 });
  s.addText([
    { text: "분석에서 ", options: { color: C.ink } },
    { text: "OCPP 호환문제가 169건", options: { bold: true, color: C.tealDk } },
    { text: "으로 최다 → 제조사별 실제 스펙을 직접 대조했습니다.\n\n", options: { color: C.ink } },
    { text: "핵심 발견: ", options: { bold: true, color: C.teal } },
    { text: "OCPP 1.6 ChargingProfile은 ‘속도 한도’ 제어용이라 목표 kWh 자동종료가 불가 → ", options: { color: C.ink } },
    { text: "서버 RemoteStop / 기기 Local Stop", options: { bold: true, color: C.tealDk } },
    { text: " 등 제조사별 우회 설계를 요금정책에 반영.", options: { color: C.ink } },
...
(() => {
  ...
  s.addShape(pres.shapes.RECTANGLE, { x: tx, y: ty, w: tw, h: 0.56, fill: { color: C.teal } });
  heads.forEach((h, i) => s.addText(h, { x: colX[i] + 0.12, y: ty, w: colWd[i] - 0.2, h: 0.56, fontFace: HF, fontSize: 12.5, bold: true, color: C.white, valign: "middle", align: i === 0 ? "left" : "center", margin: 0 }));
  let ry = ty + 0.56;
  const rh = 0.74;
  rows.forEach((r, i) => {
    s.addShape(pres.shapes.RECTANGLE, { x: tx, y: ry, w: tw, h: rh, fill: { color: i % 2 ? "EAF1EF" : C.white }, line: { color: C.line, width: 0.75 } });
...
(() => {
  ...
  s.addShape(pres.shapes.RECTANGLE, { x: tx, y: ty, w: tw, h: 0.5, fill: { color: C.teal } });
  heads.forEach((h, i) => s.addText(h, { x: colX[i] + 0.1, y: ty, w: colWd[i] - 0.15, h: 0.5, fontFace: HF, fontSize: 12, bold: true, color: C.white, valign: "middle", align: i === 0 ? "left" : "center", margin: 0 }));
  let ry = ty + 0.5;
  const rh = 0.55;
  rows.forEach((r, i) => {
    s.addShape(pres.shapes.RECTANGLE, { x: tx, y: ry, w: tw, h: rh, fill: { color: i % 2 ? "EAF1EF" : C.white }, line: { color: C.line, width: 0.75 } });
  ...
  let iy = ty + 0.78;
  rules.forEach(([h, d]) => {
    s.addShape(pres.shapes.OVAL, { x: rx + 0.4, y: iy + 0.05, w: 0.16, h: 0.16, fill: { color: C.teal } });
    s.addText([
      { text: h + "\n", options: { bold: true, color: C.ink, fontSize: 12.5 } },
      { text: d, options: { color: C.muted, fontSize: 11 } },
...
// ===================================================================
// 18.7 [사업성] 충전 유닛 이코노믹스
// ===================================================================
(() => {
  const s = pres.addSlide();
  chrome(s, "Business Case", "사업성 — 충전 유닛 이코노믹스 (가정 기반 시나리오)");
  const ty = 1.85;
  // left: margin structure
  const lx = M, lw = 5.3;
  card(s, lx, ty, lw, 2.55);
  s.addText("kWh당 마진 구조", { x: lx + 0.3, y: ty + 0.16, w: lw - 0.6, h: 0.4, fontFace: HF, fontSize: 15, bold: true, color: C.ink, margin: 0 });
  const mrow = [["판매 단가", "4,000 ₭/kWh", C.tealDk], ["− 전력 원가", "~647 ₭/kWh", C.muted], ["= 총마진", "≈ 3,350 ₭/kWh · 84%", C.amber]];
  let my = ty + 0.68;
  mrow.forEach(([k, v, col]) => {
    s.addText(k, { x: lx + 0.4, y: my, w: 2.1, h: 0.4, fontFace: BF, fontSize: 12.5, color: C.ink, valign: "middle", margin: 0 });
    s.addText(v, { x: lx + 2.4, y: my, w: lw - 2.7, h: 0.4, fontFace: HF, fontSize: 13.5, bold: true, color: col, align: "right", valign: "middle", margin: 0 });
    my += 0.5;
  });
  s.addText("라오스 전력 잉여·저가 → 충전 원가의 구조적 우위 (운영비 차감 전 총마진)", { x: lx + 0.3, y: ty + 2.0, w: lw - 0.6, h: 0.45, fontFace: BF, fontSize: 9.5, italic: true, color: C.muted, valign: "middle", lineSpacingMultiple: 1.05, margin: 0 });
  // right: payback sensitivity table
  const rx = lx + lw + 0.3, rw = W - M - rx;
  card(s, rx, ty, rw, 2.55);
  s.addText("회수 민감도 — 변수는 가동률", { x: rx + 0.3, y: ty + 0.16, w: rw - 0.6, h: 0.4, fontFace: HF, fontSize: 15, bold: true, color: C.ink, margin: 0 });
  const heads = ["가동률 (일 충전)", "연 총마진", "단순 회수"];
  const prows = [["보수 · 40 kWh", "49M ₭", "~8년", C.muted], ["기준 · 80 kWh", "98M ₭", "~4년", C.tealDk], ["낙관 · 150 kWh", "184M ₭", "~2.2년", C.amber]];
  const ctx = rx + 0.3, ctw = rw - 0.6, cwd = [ctw * 0.42, ctw * 0.29, ctw * 0.29];
  const ccx = [ctx, ctx + cwd[0], ctx + cwd[0] + cwd[1]];
  let pry = ty + 0.62;
  heads.forEach((h, i) => s.addText(h, { x: ccx[i], y: pry, w: cwd[i], h: 0.32, fontFace: BF, fontSize: 10.5, bold: true, color: C.muted, align: i ? "center" : "left", valign: "middle", margin: 0 }));
  s.addShape(pres.shapes.LINE, { x: ctx, y: pry + 0.34, w: ctw, h: 0, line: { color: C.line, width: 1 } });
  pry += 0.42;
  prows.forEach(([a, b, c, col]) => {
    s.addText(a, { x: ccx[0], y: pry, w: cwd[0], h: 0.4, fontFace: BF, fontSize: 11.5, bold: true, color: C.ink, valign: "middle", margin: 0 });
    s.addText(b, { x: ccx[1], y: pry, w: cwd[1], h: 0.4, fontFace: BF, fontSize: 11.5, color: C.ink, align: "center", valign: "middle", margin: 0 });
    s.addText(c, { x: ccx[2], y: pry, w: cwd[2], h: 0.4, fontFace: HF, fontSize: 12.5, bold: true, color: col, align: "center", valign: "middle", margin: 0 });
    pry += 0.44;
  });
  s.addText("가정: 급속충전기 1기 CAPEX ~4억₭(≈$18k) · 세션 ~10kWh · 마진 3,350₭(운영비 별도)", { x: rx + 0.3, y: ty + 2.08, w: rw - 0.6, h: 0.4, fontFace: BF, fontSize: 9, italic: true, color: C.muted, valign: "middle", margin: 0 });
  // bottom insight band
  card(s, M, ty + 2.75, W - 2 * M, 1.55, C.teal, false);
  s.addShape(pres.shapes.RECTANGLE, { x: M, y: ty + 2.75, w: 0.16, h: 1.55, fill: { color: C.white } });
  s.addText([
    { text: "결론 — 마진은 구조적으로 높다(84%). 변수는 ‘가동률’이다.\n", options: { bold: true, color: C.white } },
    { text: "그리고 ", options: { color: C.white } },
    { text: "KOKKOK Move 드라이버(현지 라이드헤일링 운영)", options: { bold: true, color: "EAFBF1" } },
    { text: "가 그 가동률을 보장한다 — LOCA가 택시앱 드라이버로 1년 내 회수한 바로 그 레버. ⚠️ 가정치이며, 보수 시나리오에서도 사업성 확보 구간.", options: { color: C.white } },
  ], { x: M + 0.45, y: ty + 2.92, w: W - 2 * M - 0.9, h: 1.3, fontFace: BF, fontSize: 13, lineSpacingMultiple: 1.18, valign: "top", margin: 0 });
})();
...
(() => {
  ...
  // header
  s.addShape(pres.shapes.RECTANGLE, { x: tx, y: ty, w: tw, h: 0.5, fill: { color: C.teal } });
  heads.forEach((h, i) => s.addText(h, { x: cx[i] + 0.12, y: ty, w: cwd[i] - 0.2, h: 0.5, fontFace: HF, fontSize: 11.5, bold: true, color: C.white, valign: "middle", margin: 0 }));
  const rows = [
    ["라오스 국한\n(진입 시장)", C.teal,
  ...
  // 결론 band
  const by = ry + 0.66;
  card(s, M, by, tw, 1.18, C.teal, false);
  s.addShape(pres.shapes.RECTANGLE, { x: M, y: by, w: 0.16, h: 1.18, fill: { color: C.white } });
  s.addText([
    { text: "결론 — LOCA EV를 경쟁사·벤치마크로 채택 ", options: { bold: true, color: C.white } },
    { text: "(동일 비즈모델·동일 시장·검증된 현지 사업성·동급 스케일).  ", options: { color: C.white } },
    { text: "보완: ", options: { bold: true, color: "EAFBF1" } },
...
(() => {
  ...
  cols.forEach(([title, col, items], i) => {
    ...
    s.addText((i + 1) + ". " + title, { x: x + 0.18, y: cy, w: cw - 0.3, h: 0.56, fontFace: HF, fontSize: 14, bold: true, color: C.white, valign: "middle", margin: 0 });
    s.addText(items.map((t) => ({ text: t, options: { bullet: { code: "2022", indent: 12 }, breakLine: true, paraSpaceAfter: 9 } })),
      { x: x + 0.26, y: cy + 0.72, w: cw - 0.48, h: ch - 0.9, fontFace: BF, fontSize: 11, color: C.ink, lineSpacingMultiple: 1.1, valign: "top", margin: 0 });
  });
  // bottom: honest data-found threat box
  card(s, M, cy + ch + 0.18, W - 2 * M, 1.15, C.teal, false);
  s.addShape(pres.shapes.RECTANGLE, { x: M, y: cy + ch + 0.18, w: 0.16, h: 1.15, fill: { color: C.red } });
  s.addText([
    { text: "데이터가 추가로 포착한 위협 — VinFast·Xanh SM (뉴스 점유율 1위, Vingroup).  ", options: { bold: true, color: "F8A8A0" } },
    { text: "차량+라이드헤일링+자본을 결합해 라오스 진입 중 — 지정 벤치마크(LOCA) 밖의 실제 경쟁자. 대응: 드라이버 앵커 수요·브랜드 중립·현지 실행 속도로 선점.", options: { color: C.white } },
  ], { x: M + 0.45, y: cy + ch + 0.32, w: W - 2 * M - 0.9, h: 0.9, fontFace: BF, fontSize: 12, lineSpacingMultiple: 1.15, valign: "top", margin: 0 });
})();
...
(() => {
  ...
  card(s, M, 5.35, W - 2 * M, 1.05, C.teal, false);
  s.addShape(pres.shapes.RECTANGLE, { x: M, y: 5.35, w: 0.16, h: 1.05, fill: { color: C.white } });
  s.addText([
    { text: "측정 가능한 목표  ", options: { bold: true, color: C.white } },
    { text: "모든 KPI는 수집한 28,890건 리뷰의 현재 baseline에서 출발합니다. ", options: { color: C.white } },
    { text: "출시 후 동일 파이프라인으로 재측정해 개선 효과를 검증할 수 있습니다.", options: { color: "EAFBF1" } },
...
(() => {
  const s = pres.addSlide();
  chrome(s, "Roadmap & Ask", "실행 로드맵 & 제안 — 단계별 투자·의사결정 게이트");
  const phases = [
    ["Phase 0", "0~3개월", "파일럿", C.amber, ["비엔티안 · KOKKOK Move 드라이버 대상 급속 N기", "앱 안정성·KYC·결제 MVP", "게이트: 가동률·크래시프리·온보딩 완료율"]],
    ["Phase 1", "3~9개월", "거점 확장", C.tealDk, ["비엔티안 네트워크 확대 + 슈퍼앱 통합", "일반 EV 오너 유입", "게이트: 충전기당 회수 궤도·재방문율"]],
    ["Phase 2", "9~18개월", "2거점", C.teal, ["사반나켓 진출(양대 거점)", "외국인 결제·다국어", "게이트: 단가 흑자 전환"]],
    ["Phase 3", "18~36개월", "규모화", C.muted, ["네트워크 규모화·로밍", "충전 데이터 기반 신사업", "게이트: 네트워크 BEP"]],
  ];
  const gap = 0.25, cw = (W - 2 * M - 3 * gap) / 4, cy = 1.85, ch = 3.35;
  phases.forEach(([p, t, name, col, items], i) => {
    const x = M + i * (cw + gap);
    card(s, x, cy, cw, ch);
    s.addShape(pres.shapes.RECTANGLE, { x, y: cy, w: cw, h: 0.78, fill: { color: col } });
    s.addText(p + " · " + t, { x: x + 0.16, y: cy + 0.08, w: cw - 0.28, h: 0.32, fontFace: BF, fontSize: 10, bold: true, color: "EAF7F4", valign: "middle", margin: 0 });
    s.addText(name, { x: x + 0.16, y: cy + 0.38, w: cw - 0.28, h: 0.34, fontFace: HF, fontSize: 14.5, bold: true, color: C.white, valign: "middle", margin: 0 });
    s.addText(items.map((t2) => ({ text: t2, options: { bullet: { code: "2022", indent: 11 }, breakLine: true, paraSpaceAfter: 8 } })),
      { x: x + 0.2, y: cy + 0.92, w: cw - 0.38, h: ch - 1.1, fontFace: BF, fontSize: 10, color: C.ink, lineSpacingMultiple: 1.08, valign: "top", margin: 0 });
    if (i < 3) s.addText("→", { x: x + cw - 0.02, y: cy + ch / 2 - 0.25, w: gap + 0.06, h: 0.5, fontFace: HF, fontSize: 18, bold: true, color: C.amber, align: "center", valign: "middle", margin: 0 });
  });
  // Ask band
  card(s, M, cy + ch + 0.18, W - 2 * M, 1.15, C.teal, false);
  s.addShape(pres.shapes.RECTANGLE, { x: M, y: cy + ch + 0.18, w: 0.16, h: 1.15, fill: { color: C.white } });
  s.addText([
    { text: "제안(Ask) — Phase 0 파일럿 승인:  ", options: { bold: true, color: C.white } },
    { text: "급속충전기 N기 CAPEX + 전담팀(기획·앱·HW협업) + N개월. 리스크는 파일럿으로 제한하고, 가동률·앱 지표·회수 궤도가 게이트를 통과할 때만 다음 단계 자본을 투입(단계적 go/no-go). ", options: { color: C.white } },
    { text: "※ 규모는 가정·제안치.", options: { color: "EAFBF1", italic: true } },
  ], { x: M + 0.45, y: cy + ch + 0.32, w: W - 2 * M - 0.9, h: 0.9, fontFace: BF, fontSize: 12.5, lineSpacingMultiple: 1.15, valign: "top", margin: 0 });
})();
...
(() => {
  ...
  card(s, M, 5.55, W - 2 * M, 0.92, C.teal, false);
  s.addText([
    { text: "배운 것  ", options: { bold: true, color: C.white } },
...
(() => {
  ...
  card(s, M, cy + ch + 0.15, W - 2 * M, 0.92, C.teal, false);
  s.addShape(pres.shapes.RECTANGLE, { x: M, y: cy + ch + 0.15, w: 0.16, h: 0.92, fill: { color: C.tealDk } });
  s.addText([
    { text: "그래서  ", options: { bold: true, color: C.white } },
...
(() => {
  ...
  s.addShape(pres.shapes.OVAL, { x: -1.8, y: 4.0, w: 5.0, h: 5.0, fill: { color: C.tealLt, transparency: 45 } });
  s.addShape(pres.shapes.OVAL, { x: W - 2.6, y: -1.6, w: 4.2, h: 4.2, fill: { color: C.tealLt, transparency: 40 } });

  s.addText("정리하며", { x: M + 0.1, y: 1.5, w: 10, h: 0.4, fontFace: BF, fontSize: 14, bold: true, color: C.teal, charSpacing: 2, margin: 0 });
  s.addText([
    { text: "데이터로 문제를 풀고,\n", options: { color: C.ink } },
    { text: "근거로 서비스를 설계했습니다.", options: { color: C.teal } },
  ...
  let py = 4.0;
  pts.forEach((t) => {
    s.addShape(pres.shapes.OVAL, { x: M + 0.1, y: py + 0.05, w: 0.2, h: 0.2, fill: { color: C.teal } });
    s.addText(t, { x: M + 0.45, y: py - 0.05, w: 11, h: 0.45, fontFace: BF, fontSize: 15, color: C.ink, valign: "middle", margin: 0 });
    py += 0.6;
  });

  s.addShape(pres.shapes.LINE, { x: M + 0.1, y: 6.25, w: W - 2 * M - 0.2, h: 0, line: { color: C.line, width: 1.5 } });
  s.addText([
    { text: "감사합니다.  ", options: { bold: true, color: C.ink, fontSize: 18 } },
    { text: "Q & A", options: { color: C.teal, bold: true, fontSize: 18 } },
  ], { x: M + 0.1, y: 6.5, w: 7, h: 0.5, fontFace: HF, valign: "middle", margin: 0 });
  s.addText("코코넛사일로 · KOKKOK EV  |  마수한 · 김재희  |  2026-06-19", {
    x: W - 6.6, y: 6.5, w: 6, h: 0.5, fontFace: BF, fontSize: 11, color: C.muted, align: "right", valign: "middle", margin: 0,
  });
})();
...
(() => {
  ...
  s.background = { color: C.white };
  s.addShape(pres.shapes.RECTANGLE, { x: 0, y: 0, w: W, h: 0.14, fill: { color: C.teal } });
  s.addShape(pres.shapes.OVAL, { x: W - 3.0, y: -1.8, w: 4.6, h: 4.6, fill: { color: C.tealLt, transparency: 45 } });
  s.addText("APPENDIX", { x: M + 0.1, y: 1.45, w: 8, h: 0.4, fontFace: BF, fontSize: 14, bold: true, color: C.teal, charSpacing: 3, margin: 0 });
  s.addText("부록 — 단계별 산출물", { x: M + 0.05, y: 1.9, w: 11, h: 1.0, fontFace: HF, fontSize: 40, bold: true, color: C.ink, margin: 0 });
  s.addText("데이터 분석 → 기획 → 설계 전 과정에서 실제로 만든 산출물 + 용어 사전입니다. (발표 시 생략 · 열람·Q&A용)", { x: M + 0.1, y: 3.0, w: 11.5, h: 0.4, fontFace: BF, fontSize: 14, color: C.muted, margin: 0 });
  ...
  groups.forEach(([title, col, items], i) => {
    ...
    s.addText(title, { x: x + 0.3, y: gy, w: gw - 0.4, h: 0.4, fontFace: HF, fontSize: 17, bold: true, color: C.ink, margin: 0 });
    s.addText(items.map((t) => ({ text: t, options: { bullet: { code: "2022", indent: 14 }, breakLine: true, paraSpaceAfter: 6, color: C.ink } })),
...
// ===================================================================
// APPENDIX — 수집 데이터 인벤토리 (전수)
// ===================================================================
(() => {
  const s = pres.addSlide();
  chrome(s, "Appendix · Data Inventory", "수집 데이터 인벤토리 — 무엇을·왜·어떻게 활용했나");
  const tx = M, ty = 1.66, tw = W - 2 * M;
  const heads = ["대분류", "소분류", "이름(테이블)", "수집 범위", "수집량", "수집 목적", "활용"];
  const cwd = [1.15, 1.4, 1.95, 1.95, 1.15, 1.95, tw - 1.15 - 1.4 - 1.95 - 1.95 - 1.15 - 1.95];
  const cx = [tx]; cwd.forEach((w, i) => { if (i < cwd.length - 1) cx.push(cx[i] + w); });
  const rows = [
    ["VOC", "앱스토어 리뷰", "app_reviews", "라·베·태·글로벌 5개 앱", "28,890", "충전·모빌리티 사용자 불만 정량화", "감성·2계층 분류·PRD 근거", C.teal],
  ...
  s.addShape(pres.shapes.RECTANGLE, { x: tx, y: ty, w: tw, h: 0.42, fill: { color: C.teal } });
  heads.forEach((h, i) => s.addText(h, { x: cx[i] + 0.1, y: ty, w: cwd[i] - 0.16, h: 0.42, fontFace: HF, fontSize: 9.5, bold: true, color: C.white, valign: "middle", margin: 0 }));
  let ry = ty + 0.42;
  const rh = 0.62;
  rows.forEach((r, i) => {
    s.addShape(pres.shapes.RECTANGLE, { x: tx, y: ry, w: tw, h: rh, fill: { color: i % 2 ? "EAF1EF" : C.white }, line: { color: C.line, width: 0.6 } });
    s.addShape(pres.shapes.RECTANGLE, { x: tx, y: ry, w: 0.06, h: rh, fill: { color: r[7] } });
    s.addText(r[0], { x: cx[0] + 0.12, y: ry, w: cwd[0] - 0.16, h: rh, fontFace: BF, fontSize: 9.5, bold: true, color: C.ink, valign: "middle", lineSpacingMultiple: 1.0, margin: 0 });
    for (let j = 1; j <= 6; j++) {
      s.addText(r[j], { x: cx[j] + 0.1, y: ry, w: cwd[j] - 0.16, h: rh, fontFace: j === 2 ? "Consolas" : BF, fontSize: j === 4 ? 9.5 : 9, bold: j === 4, color: j === 4 ? r[7] : (j === 2 ? C.ink : C.muted), valign: "middle", lineSpacingMultiple: 1.05, margin: 0 });
    }
    ry += rh;
  });
  s.addText([
...
pres.writeFile({ fileName: path.join(__dirname, "KOKKOK_EV_NAVER테마.pptx") }).then((fn) => {
