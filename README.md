...
> 발표자료 통합 직후 진행한 **경쟁사 공식 단가·IR 보강** 과정에서, 기존 분석에 섞여 있던 **검증 안 된 단가 추정치**를 발견하고 공식 자료로 교정한 이력입니다. "데이터로 근거를 만든 PM"의 핵심은 **자기 분석의 오류도 데이터로 잡아내는 것**이라는 판단으로 기록합니다.
...
> 💡 **핵심 교훈**: 모든 변경에는 "왜"가 데이터로 설명됐습니다. 특히 ④의 재분류는 *임시 추정치·잘못된 분모·도메인 혼재*가 분석 전체를 어떻게 오염시키는지 보여주며, **자기 분석의 오류조차 데이터로 잡아내는 것**이 "데이터로 근거를 만든 PM"의 핵심임을 입증합니다.
