...
| 구분 | 소스 | 대상 | 수집량 | 담당 |
|------|------|------|--------|------|
| 앱스토어 리뷰 | Google Play | Green SM (라오스·베트남 공용) | **26,603건** ✅ | 마수한 |
| 앱스토어 리뷰 | Google Play | LOCA EV (라오스) | **7건** ✅ (현지 앱 특성상 리뷰 없음) | 마수한 |
| 앱스토어 리뷰 | Google Play | PTT blueplus+ (태국 비교군) | **1,518건** ✅ | 마수한 |
| 앱스토어 리뷰 | Google Play | PEA VOLTA (태국 비교군) | **499건** ✅ | 마수한 |
| 앱스토어 리뷰 | Google Play | EleXA / EGAT (태국 비교군) | **263건** ✅ | 마수한 |
| 유튜브 | YouTube Data API v3 | EV 충전 관련 영상 — **하이브리드 방식** | **21개 영상 / 댓글 527건 / 자막 2,625세그먼트** ✅ | 마수한 |
| SNS | Facebook (공개 페이지) | LOCA EV, Green SM (라오스·베트남) | 수집 예정 🔄 | 김재희, 마수한 |
| SNS | Instagram | 한국인 라오스 여행객 EV 리뷰 | 수집 예정 🔄 | 김재희, 마수한 |
| 블로그 | 네이버 블로그 | 한국인 라오스 여행객 충전 경험 | 수집 예정 🔄 | 김재희, 마수한 |
| 현지 리서치 | 직접 조사 | 라오스·인접국 EV 충전 서비스 현황 | 별도 문서 관리 | 김재희, 마수한 |
...
| 앱 | 수집량 | 평균별점 | 비고 |
|----|--------|---------|------|
| Green SM | 26,603건 | 2.56⭐ | 라오스·베트남 공용 앱, 언어별 전수 수집 |
| LOCA EV | 7건 | 5.0⭐ | 설치 10,000+이나 리뷰 문화 없음 |
| PTT blueplus+ | 1,518건 | 3.11⭐ | 태국 1위 EV 충전 네트워크 (비교군) |
| PEA VOLTA | 499건 | 2.28⭐ | 태국 4위, WBS 직접 언급 (비교군) |
| EleXA | 263건 | 2.41⭐ | EGAT 태국 국영 전기공사 (비교군) |

**유튜브 수집 ✅ 완료 — 하이브리드 방식**
- ① **직접 지정**: WBS 리서치에서 확인한 핵심 영상 6개 (LOCA EV 사용기, 라오스 인프라 현황 등)
- ② **키워드 검색**: 추가 15개 영상 자동 발굴
- **결과**: 영상 21개 / 댓글 527건 / 자막 2,625 세그먼트
  > 앱스토어와 달리 유튜브는 서비스 리뷰가 분산되어 있어 하이브리드 방식 채택

**SNS / 블로그 수집 🔄 진행 예정**
- Facebook 공개 페이지 (Meta Graph API 우선)
- 네이버 블로그 (한국인 라오스 여행객 리뷰)
...
| 단계 | 기간 | 상태 |
|------|------|------|
| 데이터 수집 파이프라인 구축 | 2026-05-31 ~ 2026-06-04 | 🔄 진행중 (앱스토어·유튜브 완료, SNS·블로그 예정) |
...
- **YouTube 수집 전략**: 유튜브는 앱스토어와 달리 서비스별 리뷰가 분산되어 있음 → **하이브리드 방식** 채택
  - WBS 리서치에서 직접 확인한 핵심 영상 ID를 1차 수집, 키워드 검색으로 추가 발굴 후 수동 필터링
- **YouTube 자막**: 라오스 현지 영상은 자동 자막 없거나 품질 낮을 수 있음 → `lo → vi → th → en → ko` 순서로 언어 우선순위 적용
- **Facebook 수집**: Selenium 크롤링은 Meta ToS 위반 소지 → Meta Graph API 우선, 불가 시 다른 소스로 대체
- **앱 리뷰 볼륨**: LOCA EV는 설치 10,000+이나 실제 리뷰 7건 — 라오스 현지 앱 특성상 리뷰 작성 문화 없음. 대신 태국 비교군(PTT·PEA·EleXA) 추가 수집으로 보완
- **Google Play 페이지네이션**: 언어·국가 조합당 약 2,000~3,000건 제한 → 베트남어·태국어·영어 등 다중 언어 조합으로 전수 수집
