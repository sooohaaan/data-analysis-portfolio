...
| **프로젝트 유형** | 핀테크 기업 연계 프로젝트 |
| **참여기업** | 코코넛사일로(주) |
| **프로젝트 참여 기간** | 2026-04-09 ~ 2026-06-19 |
| **교육 과정** | 핀테크 서비스 기획 및 데이터 분석 전문가 성장 과정(4회차)의 일환으로 진행 |
...
#### 🎓 교육 과정 — 핀테크 서비스 기획 및 데이터 분석 전문가 성장 과정 (4회차)

| 항목 | 내용 |
|------|------|
| **훈련 기간** | 2025-12-18 ~ 2026-06-19 |
| **훈련 시간** | 960시간 |
| **훈련 일수** | 120일 |
| **담당 교·강사** | 황성민 |
| **담당 운영 매니저** | 김도운 |
...
└── SNS (Meta Graph API — 보류·보조)
    ...
    └── 발표자료 — 20분 발표 deck 58장 (presentation/) ✅
```
...
> 📑 **전체 흐름을 한 문서로 보려면 → [프로젝트 종합 결과서 (수집·분석·시각화 총정리)](프로젝트_종합_결과서.md)**
...
이 프로젝트는 **두 프레임을 역할을 나눠** 정리합니다.

- **CRISP-DM** = *이 프로젝트(데이터 작업) 자체*를 설명하는 주 프레임. ※ CRISP-DM = *Cross-Industry Standard Process for Data Mining* — 산업·도구에 종속되지 않는 데이터 분석 표준 프로세스(1999, 사실상 업계 표준).
- **SDLC** = *만들 KOKKOK EV 서비스(소프트웨어)*의 어디까지를 했는지 **포지셔닝**하는 보조 렌즈.

**Phase 0~5 ↔ CRISP-DM 매핑**

| 프로젝트 Phase | CRISP-DM 단계 |
|---|---|
| Phase 0 계획 | 비즈니스 이해 (Business Understanding) |
| Phase 1~2 인프라·수집 | 데이터 이해 (Data Understanding) |
| Phase 3 전처리 | 데이터 준비 (Data Preparation) |
| Phase 4 분석·분류 | 모델링 (Modeling) |
| (가설 검증·교차검증) | 평가 (Evaluation) |
| Phase 5 기획 연계 | 배포 (Deployment) |

> 🔁 **핵심 루프**: CRISP-DM은 순환(iterative) 모델입니다. 이 프로젝트의 최대 발견인 **VOC 2계층 재분류**가 바로 *평가 → 데이터 이해로 되돌아간 루프* — "앱오류=OCPP" 가설을 **기각**하고 충전앱 #1 불만이 **앱 불안정 27.6%**임을 규명한 지점입니다. (폭포수 SDLC로는 이 반복이 표현되지 않습니다.)

**한 줄 요약** — *이 프로젝트는 **CRISP-DM으로 수행**했고, 그 산출물은 서비스 **SDLC의 계획 → 분석 → 설계** 단계에 해당한다 (개발은 범위 외).*
...
초기 가설은 "앱오류가 부정 리뷰의 다수이고, 그 원인이 앱 버그가 아니라 충전기 하드웨어와의 통신(OCPP) 문제일 수 있다"는 것이었습니다. 이 가설을 검증하기 위해 하드웨어 레이어 뉴스를 수집했습니다. EV 충전 서비스는 **소프트웨어(앱)** 와 **하드웨어(충전기기)** 두 레이어로 구성되므로, 앱 레이어 분석만으로 원인을 단정할 수 없다고 보았기 때문입니다.
...
| `충전기결함` | 75건 | 기기 불량·고장 (**Negative 32%**) |
...
| 도메인 | 비율 |
|--------|------|
| 라이드헤일링 (Green SM) | **42.4%** |
| 앱공통 | **39.2%** |
| 기타 | 16.6% |
| 충전전용 | 0.7% |

**충전앱 레이어 — 진짜 우선순위 (PTT·PEA·EleXA·LOCA, 부정 1,227건)**

| 순위 | 카테고리 | 비율 | PRD 연계 |
|------|---------|------|---------|
| 1 | **앱 불안정** | **27.6%** | 충전 세션 자동 복구 |
| 2 | **계정·인증·KYC** | **15.0%** | 간편 온보딩·KYC |
| 3 | **결제·환불·지갑** | **11.0%** | RFID 오프라인 결제 |
| 4 | 충전소위치 | 2.4% | 실시간 충전소 지도 |
| 5 | 외국인·다국어 | 1.5% | 다국어 UX |

**라이드헤일링 레이어 (Green SM, 부정 14,234건)**: 배차 21.4% · 드라이버 12.0% · 목적지 6.1%
...
**배경**: `app_reviews` 28,890건의 **92%가 베트남 EV 라이드헤일링 앱 Green SM(Xanh SM)** 리뷰였습니다. 기존 충전 중심 7개 카테고리(앱오류·결제오류·연결실패·충전속도·UI불편·충전소위치·고객서비스)로는 라이드헤일링 맥락의 불만을 담지 못해, **부정 리뷰의 50.0%가 '기타'로 빠지는** 문제가 있었습니다.

**재분류 방법론**: 도메인을 **(1)라이드헤일링 (2)충전 (3)앱공통** 으로 분리하고, 각 도메인 안에서 다국어(vi/th/en/ko) 키워드로 세분화했습니다. 결과적으로 **기타 비율이 50.0% → 16.6%** 로 떨어지며 부정 VOC의 실제 우선순위가 드러났습니다.

| 레이어 | 부정 건수 | Top 카테고리 |
|--------|---------|-------------|
| **충전앱** (PTT·PEA·EleXA·LOCA) | 1,227건 | 앱 불안정 27.6% · 계정·인증·KYC 15.0% · 결제·환불·지갑 11.0% · 충전소위치 2.4% · 외국인·다국어 1.5% |
| **라이드헤일링** (Green SM) | 14,234건 | 배차 21.4% · 드라이버 12.0% · 목적지 6.1% |

**핵심 결과**: 우리 팀(충전 파트) 관점에서 충전앱의 진짜 우선순위는 **앱 안정성 → 온보딩(KYC) → 지갑·결제** 순이며, 초기 가설이던 OCPP·하드웨어 인과는 충전앱 VOC로 뒷받침되지 않았습니다.
...
- "충전앱 앱 불안정 27.6%·온보딩(KYC) 15.0%는 기회 — 안정성·온보딩만 잡아도 동남아 2위"
...
|---------|------|-----------|---------|
| 🔴 P0 | **충전 세션 자동 복구** | 충전앱 앱 불안정 27.6% | 앱 불안정 리뷰 50% 감소 |
| 🔴 P0 | **간편 온보딩·KYC** | 충전앱 계정·인증·KYC 15.0% | 온보딩 이탈·인증 불만 감소 |
| 🔴 P0 | **QR 재시도 + 수동 ID 입력** | 연결실패 + 유튜브 실사용 사례 | 연결 실패 80% 감소 |
| 🟠 P1 | **RFID 오프라인 결제** | 충전앱 결제·환불·지갑 11.0% | 결제 불만 리뷰 40% 감소 |
...
| 🔴 P0 | **충전기 상태 실시간 모니터링** | 충전기결함 Negative 32% | 백엔드·HW |
...
| - | Figma 프로토타입 | 🔄 진행중 | - |
| - | 발표 자료 | ✅ 완료 | `presentation/` — 발표 deck **58장** (5챕터+부록 · 페르소나 인물화 · 사업성·전략 · 리스크 · 용어사전) |
| - | 최종 발표 | 📌 확정 | 2026-06-19 |

> 📊 **발표자료**: 데이터 분석 + 기획·설계 + **사업·전략** 산출물을 하나의 발표(58장)로 통합 → [`presentation/`](presentation/) 참조.
> 흐름: 거시(PEST · TAM-SAM-SOM · 경쟁사 ①②③[라오스 뉴스 점유율]) → 미시(VOC · 감성 · 포지셔닝 · 페르소나[인물화·규모추정]) → 솔루션(PRD · 여정맵 · OCPP · 요금정책) → **사업·전략(사업성 유닛이코노믹스 · 벤치마크 선정[데이터 vs 경영진] · 왜 우리가 이기나 · 실행 로드맵&Ask)** → 기대효과 · **리스크·가정·한계** · 회고 → 부록(단계별 산출물 · **용어 사전**).
> 🔄 **최근 보강**: 2계층 재분류 반영(앱 불안정 27.6%), PEST에 정책·연료·LECS7 수치, 경쟁사 ③ 라오스 SoV, 임원용 사업성·벤치마크·로드맵·리스크 슬라이드, 발표 타이틀 변경(*데이터로 문제를 풀어 서비스를 만드는 기획자*), **분석 대상 기업 9개 소개표**(경쟁사 분석 전), **Grab 시장 진입 교훈**(라오스 미진출·Xanh SM 우회), 부록 **수집 데이터 인벤토리 표**(대분류·범위·수집량·목적·활용), **데이터 요구사항 정의(DRD) 슬라이드**(CH03 — 분석질문↔데이터 매핑·수집 기준·사전 리스크 식별), **방법론 슬라이드**(CH05 회고 — CRISP-DM 순환 + 평가→데이터이해 재분류 루프 · SDLC 포지셔닝).
...
| **가구 소비·빈곤(LECS7 2024/25)** | 빈곤율 **15.0%**(도시 8.9↑) · Gini 34.7 · 월소비/인 **중위 122.6만₭·상위 335.7만₭** | EV·충전 초기 타깃=상위분위·도시(구매력) + 가격민감성 (LSB 1차, `curl -k`+pdftotext) |
| **연료·가구비용(WFP MM 2025-06)** | 휘발유 **23,834₭/L** · 디젤 19,562₭/L · 가구 MEB **월 4.98M₭** | EV 주행원가 우위(휘발유 23,834₭/L vs 충전 4,000₭/kWh) — "교통·연료" 직접 근거 (NIPN/LSB·MoIC) |

### ⚠️ LSB 수집 불가 항목 — 대체 출처 유지 (확정)

다음은 **LSB(국가통계)에 존재하지 않으므로** 기존 대체 출처를 그대로 유지합니다.

| 불가 항목 | 사유 | 대체 출처 (유지) |
|----------|------|----------------|
| **EV 충전소·충전량** | 국가통계 대상 아님(민간 인프라) | **LOCA 공식**(40→100개소) |
| **EV 전용 등록대수** | LSB 연감 EV 분리 불확실 | **Customs 4,437대**(2024.10) |
...
**🏛️ 라오스 정부 EV 정책 기조 — 강한 친(親)EV (데이터로 판정)**

- 뉴스 정책 신호 분류 결과 **친EV 다수 vs 반(反)EV 0건** → 정부는 EV를 적극 **"늘리려는"** 기조.
- 랜드마크 1차 정책을 **URL 직접 검증·DB 적재**(`news_articles`, `source='web_direct'`): 내연기관차 **수입 잠정 중단**(2026.6~12, Eco-Business) · EV 소비세 면제·0% 수입관세·등록세 30%↓(MarkLines) · **2030년 EV 30%·충전소 500개**(MEM) · 가격 통제(MOIC).
...
## 🔍 데이터 품질 관리 — 경쟁사 단가 오류 발견·교정 (2026-06)

> 발표자료 통합 직후 진행한 **경쟁사 공식 단가·IR 보강** 과정에서, 기존 분석에 섞여 있던 **검증 안 된 단가 추정치**를 발견하고 공식 자료로 교정한 이력입니다. "데이터로 문제를 풀어 서비스를 만드는 기획자"의 핵심은 **자기 분석의 오류도 데이터로 잡아내는 것**이라는 판단으로 기록합니다.

### ① 어떻게 발견했나 (3단계)

```
1) 지시: "경쟁사 공식 단가·IR을 찾아라"
   → 웹 리서치로 공식 단가 수집
     PTT 4.5~7.5฿(TOU) · PEA 5.3~8.8฿ · EleX 7.5฿ · V-Green 3,858₫ · LOCA 4,300₭
     (상장사 IR: Grab GMV $6.08B/Q4'25 · GoTo GTV Rp15.7T/Q1'25)

2) 점검: "수집 내용을 기존 분석에 반영할 필요가 있나?"
   → ev_charging_fee_policy.md 확인 중 불일치 발견
     기존 벤치마크 "PTT 9฿/kWh" ⟷ 공식 "PTT 4.5~7.5฿" → 9฿는 과대 추정

3) 전수 점검: "분석 리포트 전체에 영향 없나?"
   → grep "9 Baht|330원|340원" 로 전 리포트 스캔
   → 시장조사 PEST의 "LOCA 약 340원/kWh"도 같은 9฿에서 파생된 값임을 추가 발견
```
...
| 위치 | 기존 (오류) | 출처 문제 |
|------|------------|----------|
| `ev_charging_fee_policy.md` §1-2 | PTT·LOCA = **9 Baht/kWh ≈ 330원** | 유튜브 자막(`cuFkQBe26mM`) **추정치** — 미검증 2차 자료 |
| `ev_charging_fee_policy.md` §3·§5 | KOKKOK 급속 9฿ · 수익 810K~1.2M฿/월 | 위 9฿가 **단가·수익 시뮬·세그먼트 요금까지 전파** |
| `market_research_report.md` PEST | LOCA "약 340원/kWh" | 9฿(≈340원)에서 환산된 **같은 뿌리의 값** |

**근본 원인**: 초기 분석 시 공식 단가를 구하기 어려워 **유튜브 자막의 추정 단가(9฿)** 를 임시 벤치마크로 사용 → 이후 단가가 등장하는 여러 문서로 **검증 없이 전파**됨. 또한 최종 정책은 **4,000₭(KIP)** 인데 분석 문서는 **9฿(Baht)** 라 통화·수치가 이중으로 어긋나 있었음.

### ③ 어떻게 고쳤나

| 위치 | 교정 후 | 근거 |
|------|---------|------|
| 경쟁사 단가표 | PTT 4.5~7.5฿ · PEA 5.3~8.8฿ · EleX 7.5฿ · V-Green 3,858₫ · LOCA 4,300₭ | 각 사 **공식 고지**(PTT·PEA·EGAT·VinFast) |
| KOKKOK 급속 단가 | 9฿ → **4,000₭/kWh** | **최종 정책안**(`정책문서.docx`)과 일치 |
| 수익 시뮬레이션 | 810K~1.2M฿/월 → **360M~540M₭/월** | 4,000₭ 기준 재계산(약 30%↓ 보수적) |
| PEST 단가 | 약 340원 → **4,300₭ ≈ 265원** | 공식 LOCA 단가 재환산 |

### ④ 왜 고쳤나

1. **신뢰도 원칙 위배**: 본 프로젝트는 "공식 1차 자료만 사용"이 원칙(시장조사업체 추정치도 전량 배제). 유튜브 자막 추정 9฿는 이 원칙에 어긋나는 미검증 값.
2. **단일 소스 일관성**: 같은 단가가 3개 문서에 서로 다르게(9฿ / 340원 / 4,000₭) 존재 → 공식 단가로 통일해 모순 제거.
3. **결론의 정확성**: 단가는 **가격 포지셔닝·수익성**의 직접 근거. 다만 교정 후에도 **KOKKOK 4,000₭ < LOCA 4,300₭ 가격 우위 결론은 불변** — 오히려 근거가 정확해짐.
...
## 🧪 프로젝트 진행 기록 — 문제 → 시도 → 결과 (발표 회고용)

> 계획대로 된 것은 거의 없었습니다. 각 단계에서 **어떤 문제가 발생했고 → 어떻게 해결을 시도했으며 → 결과가 어떻게 됐는지**를 빠짐없이 기록합니다. (발표 '회고' 슬라이드의 근거 자료)

### ① 인프라 · 데이터 수집
| 문제 | 시도 / 해결 방식 | 결과 |
|------|----------------|------|
| Supabase 접속 실패 (`FATAL: tenant not found`) | 서울 리전 pooler를 입력했으나 프로젝트는 싱가포르 → **Session Pooler**(`ap-southeast-1`, 5432)로 전환 | 연결 성공. IPv4 환경은 Direct(IPv6) 대신 Session Pooler 필요 |
| Google Play 리뷰가 ~2,000건에서 멈춤 | `count`를 키워도 토큰 소진 → **언어·국가 5조합 분할 수집** | Green SM 26,603건 확보 |
| LOCA EV 리뷰 7건뿐 (통계 불가) | 라오스 리뷰 문화 부재 → **태국 선도 3개 앱(PTT·PEA·EleXA) 비교군 추가** | 분석 가능 표본 + 동남아 비교 구도 확보 |
| Green SM = V-Green 동일 앱 의심 | 앱 ID(`com.gsm.customer`) 일치 확인 → `country='MUL'` 통합 | 중복 수집 방지 |
| `youtube-transcript-api` AttributeError | 버전업으로 클래스→인스턴스 메서드 변경 → `api.list()` 사용 | 자막 2,625세그먼트 수집 |
| 네이버 블로그 OR 필터에 무관 글 대량 유입 | 1차 OR(412건)→2차 AND(335건)→**3차 title AND + 경험단어(42건)** | 노이즈 제거. 핵심 소스는 앱·유튜브로 판단 |
| 뉴스가 VOC와 목적이 혼재 | 분석 목적이 다름 인식 → **`news_articles` 테이블 분리 설계** | VOC(경험) / 시장정보 분리 |

### ② 전처리 · 분석 인프라
| 문제 | 시도 / 해결 방식 | 결과 |
|------|----------------|------|
| 전체 영어 번역이 12시간 병목 | 600건 처리 후 중단 → **XLM-RoBERTa 다국어 직접 감성분석** | 2시간으로 단축, 번역 없이 100개 언어 처리 |
| Supabase 10초 timeout (대량 UPDATE) | **200건 배치 UPDATE** + 집계는 Python/pandas로 이관 | timeout 회피, 안정적 적재 |
| 스키마 컬럼 누락(`lang_detected` 등) | 실행 중 오류 발견 → `ALTER TABLE IF NOT EXISTS` 즉시 추가 | 무중단 보강 |

### ③ 외부 데이터 · 신뢰도
| 문제 | 시도 / 해결 방식 | 결과 |
|------|----------------|------|
| Meta 토큰이 앱 토큰이라 본문 조회 불가(code 2500) | 사용자 토큰 발급 반복 실패 → **IG 수집 B안(보류)** | 앱·유튜브·뉴스 38,600건으로 분석 완결, 노트북 06 보존 |
| 단가 "9 Baht/kWh"가 미검증 추정치 | 공식 단가 수집 중 발견 → 전 문서 grep → **공식 단가·4,000₭로 교정** | 3개 문서 수치 통일, 품질관리 이력 기록 |
| 라오스 통계청(LAOSIS) `curl` 차단(SSL+JS 세션) | `.do`는 공개 API 없음 → **브라우저 자동화로 `statHtml.do` 추출** | 도별 인구·가구·연령 확보 (차량은 ASEANstats) |
| LAOSIS SPA 캡처 실패(`document_idle` 무발생) | 홈 인기통계 **직링크 `statHtml.do` 우회** | 통계표 추출 성공 |

### ④ 분석 정합성 — 이 프로젝트 최대 발견
| 문제 | 시도 / 해결 방식 | 결과 |
|------|----------------|------|
| 부정 리뷰의 '기타'가 **50%** | 규칙 사전이 좁다고 의심 → DB 표본 직접 분석 | **근본 원인 = Green SM(92%)이 충전 앱이 아니라 베트남 라이드헤일링 앱(Xanh SM)** → 배차·드라이버 불만이 충전 카테고리에 안 맞아 기타로 빠짐 |
| 핵심 지표가 문서마다 불일치(앱오류 35% vs 58.6%, 결제 16.8% vs 8.2%) | 분모 차이(기타 포함/제외) + 두 번의 분류 실행 잔재로 진단 | **도메인 분리 재분류로 근본 해결** |
| 분류 코드가 버전관리에 없어 재현 불가 | `src/review_classifier.py` + `notebooks/09` 로 복원 | 재현성 확보 |
| 재분류 — 다국어 키워드 5차 반복 튜닝 | (1)라이드헤일링 (2)충전 (3)앱공통 도메인 3분할 + vi/th/en 보강 | **기타 50%→16.6%**, 충전앱 진짜 #1 = **앱 불안정 27.6%** 규명, "앱오류=OCPP" 가설 **기각** |
| LOCA "EV 언급 43.6%" vs 표의 72건(16.1%) 모순 | 산식 불일치 확인 → **16.1%(72/447)로 통일** | 문서·덱 수치 일치 |

### ⑤ 라오스 현지 · 경쟁 구도
| 문제 | 시도 / 해결 방식 | 결과 |
|------|----------------|------|
| 라오스 1차 데이터 공백(전부 대용 표본) | 라오스 뉴스 **점유율(SoV) 타겟 수집 419건** | VinFast·Xanh SM 21 > LOCA 17 > KOKKOK Move 3 → **벤치마크 2축**, Grab 미진출 |
| 라오어(lo) Google News 색인 0건 | 영어·한국어 보도로 대체, 한계 명시 | **점유율·인지도 프록시**로 해석 |
| Facebook 본문 대량 수집 불가(Meta API) | P2 후속(페이지 공개지표·관찰)으로 연기 | 한계 슬라이드에 명시 |

### ⑥ 형상관리(PR)
| 문제 | 시도 / 해결 방식 | 결과 |
|------|----------------|------|
| `--delete-branch`로 스택 PR(#17)이 자동 CLOSED | base 브랜치 삭제로 PR 체인 끊김 → **로컬 병합으로 복구** | #15·#16·#17 작업 모두 main 반영, 빌드 검증 |

> 💡 **핵심 교훈**: 모든 변경에는 "왜"가 데이터로 설명됐습니다. 특히 ④의 재분류는 *임시 추정치·잘못된 분모·도메인 혼재*가 분석 전체를 어떻게 오염시키는지 보여주며, **자기 분석의 오류조차 데이터로 잡아내는 것**이 "데이터로 문제를 풀어 서비스를 만드는 기획자"의 핵심임을 입증합니다.
