...
| 유튜브 | 댓글 + 자막(캡션) | 라오스·베트남·한국 EV 관련 영상 — **하이브리드 방식** | 마수한 |
...
- 앱스토어 스크래퍼 (Python) → DB 자동 적재 ✅ (Green SM 500건 + V-Green 500건)
- YouTube Data API v3 댓글·자막 수집 — **하이브리드 방식** 🔄
  - ① **직접 지정**: WBS 리서치에서 확인한 핵심 영상 6개 (LOCA EV, Green SM, 라오스·태국 현지 리뷰)
  - ② **키워드 검색**: 추가 영상 발굴 후 수동 필터링 → 확정 영상에 추가
  > 앱스토어와 달리 유튜브는 특정 서비스 리뷰가 한 곳에 모여있지 않아 하이브리드 방식 채택
- SNS / 네이버 블로그 보조 수집 🔄
