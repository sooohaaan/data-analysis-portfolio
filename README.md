...
└── SNS (Meta Graph API — 보류·보조)
    ...
    └── 발표자료 — 20분 발표 deck 57장 (presentation/) ✅
```
...
| - | Figma 프로토타입 | 🔄 진행중 | - |
| - | 발표 자료 | ✅ 완료 | `presentation/` — 발표 deck **57장** (5챕터+부록 · 페르소나 인물화 · 사업성·전략 · 리스크 · 용어사전) |
| - | 최종 발표 | 📌 확정 | 2026-06-19 |

> 📊 **발표자료**: 데이터 분석 + 기획·설계 + **사업·전략** 산출물을 하나의 발표(57장)로 통합 → [`presentation/`](presentation/) 참조.
...
> 🔄 **최근 보강**: 2계층 재분류 반영(앱 불안정 27.6%), PEST에 정책·연료·LECS7 수치, 경쟁사 ③ 라오스 SoV, 임원용 사업성·벤치마크·로드맵·리스크 슬라이드, 발표 타이틀 변경(*데이터로 문제를 풀어 서비스를 만드는 기획자*), **분석 대상 기업 9개 소개표**(경쟁사 분석 전), **Grab 시장 진입 교훈**(라오스 미진출·Xanh SM 우회), 부록 **수집 데이터 인벤토리 표**(대분류·범위·수집량·목적·활용), **데이터 요구사항 정의(DRD) 슬라이드**(CH03 — 분석질문↔데이터 매핑·수집 기준·사전 리스크 식별).
